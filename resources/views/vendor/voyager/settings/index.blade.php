...
@section('content')
    <div class="container-fluid">
        ...
        @if(config('voyager.show_dev_tips') && auth()->user()->hasRole('admin'))
    ...
    <div class="page-content settings container-fluid">
            ...
            <div class="panel">
                ...
                <div class="page-content settings container-fluid">
					...
					@if(!auth()->user()->hasRole('admin'))
						@php $settings = array_except($settings,['Admin','Site','Footer']); @endphp 
					@endif
						@foreach($settings as $group => $setting) 
						{{-- $loop->first ? 'active' : null --}}
							<li class="{{$loop->first ? 'active' : null}}"> <a data-toggle="tab" href="#{{ str_slug($group) }}">{{ $group }}</a> </li> 
						@endforeach
                    </ul>
                    ...
                    <div class="tab-content">
					   ...
					   <div id="{{ str_slug($group) }}" class="tab-pane fade in {{$loop->first ? 'active' : null}}">
                            @foreach($group_settings as $setting)
                            <div class="panel-heading">
                                <h3 class="panel-title">
                                    {{ $setting->display_name }} @if(config('voyager.show_dev_tips') && auth()->user()->hasRole('admin'))<code>setting('{{ $setting->key }}')</code>@endif
                                </h3>
								@if(auth()->user()->hasRole('admin'))
        ...
        <div style="clear:both"></div>
				...
				<div class="panel" style="margin-top:10px; @if(!auth()->user()->hasRole('admin')) display:none; @endif">
